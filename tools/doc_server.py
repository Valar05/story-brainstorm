...
class Handler(BaseHTTPRequestHandler):
 def do_GET(self):
  try:
   p=safe_path(urlparse(self.path).path[len('/doc/'):]) if self.path.startswith('/doc/') else None
   if p is None:
    body='<h1>Story Brainstorm</h1><p>Use /doc/&lt;relative path&gt;.</p>'.encode()
   else: body=('<pre>'+html.escape(p.read_text(encoding="utf-8",errors="replace"))+'</pre>').encode()
